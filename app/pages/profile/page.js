  ...
  const handleUpdateProfile = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Then update author data if needed
        if (isAuthor) {
          await fetch("/api/author/profile", {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            ...
            {activeTab === "author" && isAuthor && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {Object.entries(authorData.socialLinks).map(([key, value]) => (
    ...
    <div className="bg-white rounded-xl shadow-sm">
      ...
      <div className="divide-y">
        {recentArticles?.map((article) => (
          <div key={article._id} className="p-4 hover:bg-gray-50 transition">
